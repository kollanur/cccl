...
double run_adjacent_difference_benchmark_hpx(int const test_count, ExPolicy policy, FwdIter1 first, FwdIter2 last, FwdIter3 dest) {
  // warmup
  hpx::adjacent_difference(policy, first, last, dest);
...
static void basic(nvbench::state& state, nvbench::type_list<T>)
{
  ...
  const double seq_time = run_adjacent_difference_benchmark_hpx(10, hpx::execution::seq, input.cbegin(), input.cend(), output.begin());
  ...
  auto temp = run_adjacent_difference_benchmark_hpx(10, hpx::execution::par.with(nc, mnc), input.cbegin(), input.cend(), output.begin());
  ...
  state.exec(nvbench::exec_tag::gpu | nvbench::exec_tag::sync, [&](nvbench::launch& launch) {
    thrust::adjacent_difference(policy(alloc, launch).with(acs, collect_params), input.cbegin(), input.cend(), output.begin());
  });
